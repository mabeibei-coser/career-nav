...
const FALLBACK_GENERATED: QuizQuestion[] = [
  {
    id: "SJT-02",
...
function normalizeGeneratedQuestions(data: LLMGeneratedBank): QuizQuestion[] {
  return data.questions.map((q, i) => ({
    id: `SJT-0${i + 2}`, // SJT-02 to SJT-08，强制覆盖 LLM 可能乱填的 id
    text: q.text.trim(),
...
async function generateSJTQuestions(formData: Partial<JobFormData>): Promise<QuizQuestion[]> {
 ...
 *
 * 返回 8 道情境判断题：
 *   Q1（SJT-01）: 固定题目，来自 data/quiz-bank.json
 *   Q2-Q8（SJT-02 to SJT-08）: LLM 根据求职者背景动态生成；失败时使用兜底题
 */
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      // 兼容新（body.formData）和旧（body.identity）两种调用方式
    ...
    // E2E Mock 模式：直接返回固定题 + 兜底题，不消耗 LLM
